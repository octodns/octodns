...
- `octodns/manager.py:Manager` — central orchestrator for config/plugins and end-to-end sync/dump/compare.
- `octodns/provider/base.py:BaseProvider` — provider lifecycle: populate → desired/existing preprocessing → diff → `Plan` creation.
- `octodns/provider/plan.py:Plan` — plan representation, deterministic change ordering, and safety checks.
...
- `octodns/zone.py:Zone.apply(changes)` — applies a change list to a Zone copy.
- `octodns/record/base.py:Record.new` — factory selecting concrete record type from the registry; validates and normalizes IDNA.
- `octodns/record/base.py:Record` + value types — record serialization/validation, with optional templating-driven parsing/validation/rendering.
- `octodns/provider/plan.py:Plan` — holds `changes`, computed counts, `meta`, and `raise_if_unsafe()` safety checks; `changes` are sorted deterministically.
- `octodns/provider/plan.py:Plan.data` — checksum/serialization-friendly structure (stable keys/order) used by checksum-gated apply.
...
3. For each zone/target:
   ...
   - Processors run in phases (including two-zone phase) with lenient-mode behavior.
...
Processor hook return contract (important): hooks should return the modified `Zone`/`Plan` objects (don’t return `None` unless explicitly documented).
...
- Run focused suites:
  ...
  - Processors (examples):
    ...
    - `pytest -q tests/test_octodns_processor_trailing_dots.py`
...
- Lenient-mode is record-level and hook-level:
  ...
  - Some processors intentionally ignore the hook’s `lenient` parameter and use `record.lenient` (e.g., `octodns/processor/spf.py:SpfDnsLookupProcessor`).
...
- Root NS safety semantics:
  - `octodns/provider/plan.py:Plan.raise_if_unsafe()` treats root NS changes as unsafe (special-case rule).
- Checksum-gated apply:
  - If checksum gating is enabled, it uses `Plan.data` for a stable SHA256. Don’t change `Plan.data` shape/ordering casually.
...
- `octodns/zone.py:Zone.apply` — canonical way to apply computed change lists.
...
- Breaking processor hook contracts:
  - Don’t return `None` unless documented; sync expects zone/plan objects back.
...
- Zones use IDNA-normalized keys internally; don’t treat `zone.name` as UTF-8/unencoded.
...
- `Plan.changes` is sorted deterministically at construction.
- `Plan.data` has stable keys and ordering suitable for checksums.
...
- If you change CI-facing behavior, inspect `.github/workflows/main.yml` and related workflows in your environment to confirm the actual matrix (tooling output above did not extract it fully).
