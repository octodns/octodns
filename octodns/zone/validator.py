...
class ZoneValidatorRegistry:
    ...
    def enable_sets(self, sets: set[str]) -> None:
        self.configured = True
        self.active.clear()
        sets_set = set(sets)
        for validator in self.available.values():
            if validator.sets is None or sets_set & validator.sets:
...
class ValidationReason:
    def __init__(self, reason: str, records: list[Any]) -> None:
        self.reason = reason
        self.records: set[Any] = set(records)
...
class ZoneValidator:
    ...
    def __init__(self, id: str, sets: set[str] | None = None) -> None:
        '''
